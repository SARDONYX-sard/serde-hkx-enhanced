...
use crate::{AnyError, logger::LogLevel};
    ...
    if let Some(command) = args.subcommand {
        match command {
            SubCommands::Convert(args) => {
                Ok(convert(&args.input, args.output, args.format, args.runtime).await?)
            }
            SubCommands::Tree(args) => Ok(write_tree(args.input, args.output).await?),
            SubCommands::Dump(args) => match args.reverse {
                true => Ok(hexdump::to_bytes(args.input, args.output).await?),
                false => Ok(hexdump::to_string(args.input, args.output).await?),
            },
